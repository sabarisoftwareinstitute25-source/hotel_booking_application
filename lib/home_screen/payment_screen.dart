...
class _PaymentScreenState extends State<PaymentScreen>
  ...
  void _processPayment() async {
    ...
    try {
      ...
      final checkInDate = DateTime.now().add(const Duration(days: 1));
      final checkOutDate = checkInDate.add(const Duration(days: 4));
      ...
      final bookingResponse = await _apiService.createBooking(bookingRequest);
      ...
      if (!bookingResponse.success || bookingResponse.data == null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Failed to create booking: ${bookingResponse.message}"),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      _bookingId = bookingResponse.data!.id;

      // Then, process payment
      final paymentMethod = _paymentMethods[_selectedPaymentMethod]['name'] ?? 'CARD';
      ...
      final paymentResponse = await _apiService.processPayment(paymentRequest);
      ...
      if (paymentResponse.success && paymentResponse.data != null) {
        _transactionId = paymentResponse.data!.transactionId;
        _showSuccessDialog();
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
