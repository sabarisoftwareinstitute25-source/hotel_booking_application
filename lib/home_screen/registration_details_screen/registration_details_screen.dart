  ...
  Future<void> _removeCurrentVillaFromList() async {
    try {
      ...
      if (villaIdToRemove.isEmpty) {
        print('ERROR: Villa ID is empty! Cannot remove.');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error: Could not identify villa to remove'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      print('Total users found: ${usersList.length}');
      ...
      if (userData.containsKey('villlas') && userData['villlas'] is List) {
        ...
        // Print all villa IDs for debugging
        print('Current villas:');
        for (var villa in villasList) {
          ...
          String currentId = villaMap['id']?.toString() ??
              ...
              'NO ID';
          String currentName = villaMap['basicInfo']?['villaName']?.toString() ??
              villaMap['villaName']?.toString() ??
              'Unknown';
          print('  - ID: "$currentId", Name: $currentName');
        }

        // Remove the specific villa
        final int initialCount = villasList.length;
        ...
        print('Removed ${initialCount - villasList.length} villa(s)');
        print('Remaining villas count: ${villasList.length}');

        // Update user data
        userData['villlas'] = villasList;

        // If no villas left, you might want to remove the user or keep empty list
        if (villasList.isEmpty) {
          print('No villas left. Keeping user with empty list.');
        }
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Villa removed successfully'),
            backgroundColor: Colors.green,
          ),
        );
      } else {
        print('No villas list found in user data');
      }
    } catch (e) {
      print('Error removing villa: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error removing villa: ${e.toString()}'),
          backgroundColor: Colors.red,
        ),
      );
    }
...
class _ApartmentRegistrationDetailsScreenState extends State<ApartmentRegistrationDetailsScreen>
  ...
  void initState() {
    ...
    // Add debug print to see what data is loaded
  ...
  // Add these methods for missing fields
